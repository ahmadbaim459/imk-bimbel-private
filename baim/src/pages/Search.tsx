...
interface TeacherProfile {
  ...
  teacher_subjects: { subjects: { name: string } }[];
  teacher_levels: { education_levels: { name: string } }[];
}
...
export default function Search() {
  ...
  useEffect(() => {
    fetchFilters();
    fetchTeachers(city, subject, level);
  }, []);
  ...
  const fetchTeachers = async (currentCity: string, currentSubject: string, currentLevel: string) => {
    setLoading(true);
    try {
      let query = supabase
        .from('teacher_profiles')
        .select(`
          id, full_name, photo_url, description, hourly_rate,
          cities ( city_name ),
          teacher_subjects ( subjects ( name ) ),
          teacher_levels ( education_levels ( name ) )
        `)
        .eq('verified', true);

      // We handle filtering locally for relations since Supabase JS relational filtering can be tricky
      const { data, error } = await query;

      if (error) throw error;
      
      let filteredData = data as any as TeacherProfile[];
      
      if (currentCity) {
        filteredData = filteredData.filter(t => t.cities?.city_name === currentCity);
      }
      if (currentSubject) {
        filteredData = filteredData.filter(t => 
          t.teacher_subjects.some(ts => ts.subjects?.name === currentSubject)
        );
      }
      if (currentLevel) {
        filteredData = filteredData.filter(t => 
          t.teacher_levels.some(tl => tl.education_levels?.name === currentLevel)
        );
      }
    ...
    <div className="flex-1 px-4 sm:px-8 py-8 flex flex-col md:flex-row gap-8 max-w-7xl mx-auto w-full">
      ...
      <section className="flex-1">
        ...
        <div className="grid grid-cols-1 gap-4">
          {loading ? (
            <div className="text-center py-12 text-slate-500">Memuat data guru...</div>
          ) : teachers.length === 0 ? (
            <div className="text-center py-12 text-slate-500">Tidak ada guru yang ditemukan.</div>
          ) : (
            teachers.map((teacher, idx) => (
              <div key={teacher.id} className="bg-white rounded-2xl p-4 border border-slate-200 flex flex-col sm:flex-row gap-5 hover:border-indigo-300 transition-all">
                <div className="relative shrink-0">
                  <div className="w-24 h-24 sm:w-32 sm:h-32 bg-slate-200 rounded-xl overflow-hidden flex items-center justify-center border-4 border-slate-50 shadow-inner">
                    {teacher.photo_url ? (
                      ...
                      <span className="text-slate-400 font-bold uppercase text-xs text-center">Tanpa Foto</span>
                    )}
                  </div>
                  {idx === 0 && (
                ...
                <div className="flex-1 flex flex-col">
                  <div className="flex justify-between mb-1">
                    <h3 className="text-lg font-bold text-slate-800">{teacher.full_name}</h3>
                    <span className="text-indigo-600 font-bold whitespace-nowrap">Rp {teacher.hourly_rate?.toLocaleString('id-ID') || 0} <span className="text-xs text-slate-400 font-normal">/ jam</span></span>
                  </div>
                  
                  <p className="text-sm text-slate-500 mb-3">{teacher.description || 'Belum ada deskripsi.'}</p>
                  
                  <div className="flex flex-wrap gap-2 mb-4">
                    {teacher.teacher_subjects?.map(s => (
                      <span key={s.subjects.name} className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-[10px] font-bold uppercase">{s.subjects.name}</span>
                    ))}
                    {teacher.teacher_levels?.map(l => (
                      <span key={l.education_levels.name} className="px-2 py-1 bg-indigo-50 text-indigo-600 rounded text-[10px] font-bold uppercase">{l.education_levels.name}</span>
                    ))}
                  </div>
                  ...
                  <div className="flex justify-between items-center mt-auto">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs font-bold text-slate-400">({idx === 0 ? '12' : '0'} Review)</span>
                    </div>
                    <button className="flex items-center gap-2 bg-[#25D366] hover:bg-[#20b858] transition-colors text-white px-4 sm:px-5 py-2 rounded-lg text-sm font-bold shadow-sm">
