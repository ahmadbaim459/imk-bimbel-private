...
export default function Search() {
  ...
  const fetchTeachers = async () => {
    ...
    try {
      ...
      if (city) {
        filteredData = filteredData.filter(t => t.cities?.city_name === city);
      }
      if (subject) {
        filteredData = filteredData.filter(t => 
          t.teacher_subjects.some(ts => ts.subjects?.name === subject)
        );
      }
      if (level) {
        filteredData = filteredData.filter(t => 
          t.teacher_levels.some(tl => tl.education_levels?.name === level)
        );
      }
